...
export type StripKey<K, Str extends string> = K extends `${Str}${infer Rest}`
  ? Rest
  : never;

export type AppendToKeys<T, Str extends string> = keyof T extends string
  ? {
      [K in `${keyof T}${Str}`]: K extends `${infer S}${Str}`
        ? S extends keyof T
          ? T[S]
          : never
        : never;
...
export type PrependToKeys<T, Str extends string> = keyof T extends string
  ? {
      [K in `${Str}${keyof T}`]: K extends `${Str}${infer S}`
        ? S extends keyof T
          ? T[S]
          : never
        : never;
...
export type SettableProps<T> = {
  [K in Uncapitalize<
    StripKey<keyof AllSetters<T>, "set">
  >]: `set${Capitalize<K>}` extends keyof SettersValues<T>
    ? SettersValues<T>[`set${Capitalize<K>}`]
    : never;
};

export type Args<T> = T extends new (...args: unknown[]) => unknown
  ? ConstructorParameters<T> | ConstructorParameters<T>[0]
  : T;
...
export type EventHandlerProps<T> = PrependToKeys<
  {
    [key in Capitalize<EventHandlerKeys<T>>]: ListenerTypeForKey<
      T,
      Uncapitalize<key>
    >;
  },
...
export type Node<T, P> = Partial<SettableProps<T>> &
  Partial<EventHandlerProps<T>> &
  NodeProps<T, P>;
...
type ExtractIntrinsicElements<T> = keyof T extends string
  ? {
      [K in Uncapitalize<keyof T>]: Capitalize<K> extends keyof T
        ? Node<ConstructedObject<T[Capitalize<K>]>, T[Capitalize<K>]>
        : never;
    }
  : never;

type BaseElements = ExtractIntrinsicElements<typeof OL>;
type LayerElements = AppendToKeys<
  ExtractIntrinsicElements<typeof OLLayers>,
  "Layer"
>;
type SourceElements = AppendToKeys<
  ExtractIntrinsicElements<typeof OLSources>,
  "Source"
>;
type InteractionElements = AppendToKeys<
