...
interface Catalogue {
  [name: string]:
    ...
    | {
        [k: string]:
          | { new (...args: any): GenericOLInstance }
          | ((...args: any) => OL.Collection<GenericOLInstance>);
      };
}
...
function getConstructor(name: string) {
  ...
  const namespacedCatalogueEntry =
    candidateEnding &&
    (catalogue as any)[`*${candidateEnding}`][
      name.substring(0, name.length - candidateEnding.length)
    ];
...
const pascalCase = <T extends string>(str: T) =>
...
function getGetter<TK extends string>(instance: GenericOLInstance, prop: TK) {
  return (
    instance[`get${pascalCase(prop)}` as const]?.bind(instance) ||
    (() => instance.get(prop))
  );
}
function getSetter<TK extends string>(instance: GenericOLInstance, prop: TK) {
  return (
    instance[`set${pascalCase(prop)}` as const]?.bind(instance) ||
    ((value: any) => instance.set(prop, value))
  );
}
...
function getHaser<TK extends string>(instance: GenericOLInstance, prop: TK) {
  ...
  const arrayChecker =
    array?.includes?.bind(array) || array?.array_?.includes?.bind(array.array_);
  return (
    instance[`has${pascalCase(prop)}` as const]?.bind(instance) ||
    ((value: any) => arrayChecker(value)) ||
    (() => false)
  );
}
...
function appendChild(
  parentInstance: GenericOLInstance,
  child: GenericOLInstance
) {
  if (!child) return;
  ...
  if (child.attach) {
    ...
    child.attachAdd &&
    !getHaser(parentInstance, child.attachAdd)(child)
  ) {
...
function insertBefore(
  parentInstance: GenericOLInstance,
  child: GenericOLInstance,
  beforeChild: GenericOLInstance
) {
  if (!child) return;

  child._parent = parentInstance;
  const children = (
    getGetter(parentInstance, `${child.attachAdd}s`)() as OL.Collection<unknown>
  )["array_"] as GenericOLInstance[];
...
function switchInstance(
  ...
  [fiber, fiber.alternate].forEach((newFiber) => {
    if (newFiber !== null) {
      ...
      if (newFiber.ref) {
        if (typeof newFiber.ref === "function")
          (newFiber as unknown as any).ref(newInstance);
...
const reconciler = Reconciler({
  ...
  prepareUpdate(
    ...
    if (instance._primitive && newProps.object && newProps.object !== instance)
      return [true];
    else {
...
const render = (
  ...
  mapProps: MapProps & MapEvents
) => {
  const { view: viewProp, ...rest } = mapProps;
  const view =
    viewProp instanceof OL.View
      ? viewProp
      : { ...defaultMapProps.view, ...(viewProp || {}) };
  ...
  if (
    state.map &&
    mapProps.view instanceof OL.View &&
    mapProps.view !== state.map.getView()
  ) {
  ...
  reconciler.updateContainer(
    <context.Provider value={state as RootState}>{element}</context.Provider>,
    root,
    null,
    () => undefined
  );
...
export const MapComponent = forwardRef<
  OL.Map,
  PropsWithChildren<HTMLAttributes<HTMLDivElement> & MapProps & MapEvents>
>(function MapComponent({ children, style, view, ...rest }, ref) {
